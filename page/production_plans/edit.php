...
// Ambil data rencana
$stmt = $pdo->prepare("SELECT id, created_by, name, plan_date, target_quantity FROM production_plans WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE production_plans SET created_by = ?, name = ?, plan_date = ?, target_quantity = ? WHERE id = ?");
        $stmt->execute([$created_by, $name, $plan_date, $target_quantity, $plan_id]);
        
        // Catat log aktivitas
        $stmt = $pdo->prepare("INSERT INTO logs (user_id, action, log_time) VALUES (?, ?, NOW())");
        $stmt->execute([$_SESSION['user_id'], "Mengedit rencana produksi $name untuk tanggal $plan_date"]);
