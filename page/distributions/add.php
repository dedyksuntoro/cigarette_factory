...
if (!isset($_SESSION['user_id']) || !hasPermission($role, ['create_all', 'create_distributions'])) {
    ...
    if (empty($product_id) || empty($quantity) || empty($destination)) {
        ...
        if ($quantity > $current_stock) {
            ...
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO distributions (product_id, quantity, destination, status, created_at) VALUES (?, ?, ?, ?, NOW())");
                $stmt->execute([$product_id, $quantity, $destination, $status]);
                ...
                $stmt->execute([$_SESSION['user_id'], "Menambahkan distribusi untuk produk ID: $product_id ke $destination"]);
                ...
                $pdo->rollBack();
                $error = "Gagal menambah distribusi: " . $e->getMessage();
            }
        }
    }
}
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="product_id" name="product_id" required>
                ...
                <?php foreach ($products as $product): ?>
                    <option value="<?php echo $product['id']; ?>"><?php echo htmlspecialchars($product['product_name']) . " (Stok: {$product['stock']})"; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3">
