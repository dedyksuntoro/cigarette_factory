...
if (!isset($_SESSION['user_id']) || !hasPermission($role, ['create_all', 'create_invoices'])) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($distribution_id) || empty($amount)) {
        ...
        try {
            // Insert data faktur
            $stmt = $pdo->prepare("INSERT INTO invoices (distribution_id, amount, status, created_at) VALUES (?, ?, ?, NOW())");
            $stmt->execute([$distribution_id, $amount, $status]);

            // Catat log aktivitas
            $stmt = $pdo->prepare("INSERT INTO logs (user_id, action, log_time) VALUES (?, ?, NOW())");
            $stmt->execute([$_SESSION['user_id'], "Menambahkan faktur untuk distribusi ID: $distribution_id"]);

            header('Location: ' . $_ENV['BASE_URL'] . '/page/invoices/list.php');
            exit();
        } catch (PDOException $e) {
            $error = "Gagal menambah faktur: " . $e->getMessage();
        }
    }
}
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="distribution_id" name="distribution_id" required>
                ...
                <?php foreach ($distributions as $distribution): ?>
                    <option value="<?php echo $distribution['id']; ?>"><?php echo htmlspecialchars($distribution['destination']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3">
